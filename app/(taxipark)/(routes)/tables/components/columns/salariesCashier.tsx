...
export const SalariesCashierColumns: ColumnDef<SalariesCashier>[] =
  [
    ...
    },
    {
        ...
        header: ({ column }) => {
          return (
            <Select onValueChange={(value) => column.toggleSorting(value === 'cash')}>
...
export default function GetAllSalariesCashier() {
  const [dataSalariesCashier, setDataSalariesCashier] = React.useState<SalariesCashier[]>(
    []
  );
    ...
    async function fetchDataASalariesCashier() {
      try {
        ...
        const formattedSalariesCashierData = salariesCashierData.map(
          (salariesCashier) => ({
            ...salariesCashier,
            SalariesCashierDate: new Date(salariesCashier.SalariesCashierDate).toLocaleDateString("ru-RU"),
            salariesCashier: {
              SalariesCashierAmount: salariesCashier.SalariesCashierAmount,
              SalariesCashierComment: salariesCashier.SalariesCashierComment,
              SalariesCashierPaymentType: salariesCashier.SalariesCashierPaymentType,
            },
          })
        );
