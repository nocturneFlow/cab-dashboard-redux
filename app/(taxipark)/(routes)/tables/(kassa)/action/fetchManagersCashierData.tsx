...
export async function fetchCashRegistersCashData(
  startDate: string,
  endDate: string
) {
  try {
    const response = await fetch(
      `https://taxi-service-34d2f59aac8f.herokuapp.com/cashregisters/allByDate?startDate=${startDate}&endDate=${endDate}`,
      {
        method: "GET",
        headers: {
...
export function UseApplicationsData() {
  ...
  useEffect(() => {
    if (dateRange?.from && dateRange.to) {
      ...
      fetchCashRegistersCashData(startDate, endDate)
